...
### Task 13: Затяжной бой (добавлено после находки фаззинга в Task 12)

> **Устарело.** Счётчики `quietActions` / `idleActions` из этой задачи после финального ревью заменены обнаружением повтора позиции (`STALL_REPEATS = 3`, коммит 7695216). Актуальное правило — спека §2.3 «Затяжной бой».

Фаззинг Task 12 доказал: из некоторых достижимых позиций фазы 2 (трое активных, без козырей на руках, заходы и взятия чередуются) нет ни одной последовательности законных ходов, ведущей к концу партии. Заказчик решил: сделать поведение настройкой комнаты. Спека, раздел 2.3 «Затяжной бой».

Эта задача завершает и Task 12: незакоммиченные `src/index.ts` и `test/fuzz.test.ts` из Task 12 остаются и коммитятся здесь. Незакоммиченная правка `test/phase2.test.ts` (тест `it.skip` «KNOWN BUG…») откатывается: описанный в нём цикл на самом деле имеет выход и не является настоящей ловушкой.

**Files:**
- Modify: `packages/engine/src/types.ts`, `src/setup.ts`, `src/session.ts`, `src/result.ts`, `src/phase2.ts`
- Modify: `packages/engine/test/helpers.ts`, `test/phase2.test.ts`, `test/fuzz.test.ts`
- Keep (from Task 12, uncommitted): `packages/engine/src/index.ts`

**Interfaces:**
- Produces:
  - `type StallRule = 'forcedVidbiy' | 'endGame'` (в `types.ts`)
  - `GameState.stallRule: StallRule`, `GameState.quietActions: number`
  - событие `{ type: 'stall'; rule: StallRule }`
  - `GameSetup.stallRule?: StallRule` (по умолчанию `'forcedVidbiy'`)
  - `setupNextGame(se, playerIds, deckSize, seed, stallRule: StallRule = 'forcedVidbiy')`
  - `STALL_CIRCLES = 2` (экспорт из `phase2.ts`)
  - `finish(s, result, events)` становится экспортируемой из `result.ts`

- [ ] **Step 1: Откатить `phase2.test.ts` к закоммиченной версии**

Run: `git checkout -- packages/engine/test/phase2.test.ts`

- [ ] **Step 2: Типы**

В `packages/engine/src/types.ts`:
- после `export type Phase = …` добавить:
```ts
export type StallRule = 'forcedVidbiy' | 'endGame';
```
- в `interface GameState` после `result: GameResult | null;` добавить:
```ts
  stallRule: StallRule;
  quietActions: number; // подряд идущие действия фазы 2 без побития
```
- в `type GameEvent` добавить вариант:
```ts
  | { type: 'stall'; rule: StallRule }
```

- [ ] **Step 3: Хелперы тестов**

В `packages/engine/test/helpers.ts`:
- импорт типов: `import type { Action, GameState, PlayerState, StallRule } from '../src/types';`
- в `baseState` в возвращаемый объект добавить `stallRule: 'forcedVidbiy', quietActions: 0,`
- `phase2State` принимает необязательный `stallRule?: StallRule` в объекте опций и кладёт `stallRule: o.stallRule ?? 'forcedVidbiy'` в результат.

- [ ] **Step 4: Падающие тесты**

Дописать в конец `packages/engine/test/phase2.test.ts` (и добавить `StallRule` в импорт типов: `import type { StallRule } from '../src/types';`):
```ts
...
Run: `npx vitest run packages/engine/test/phase2.test.ts`
Expected: FAIL — `quietActions` undefined / нет события `stall`.

- [ ] **Step 6: Создание партии и сессия**

`packages/engine/src/setup.ts`: импорт `import type { GameState, PlayerId, PlayerState, StallRule } from './types';`; в `interface GameSetup` добавить `stallRule?: StallRule;`; в объект, возвращаемый `createGame`, добавить `stallRule: setup.stallRule ?? 'forcedVidbiy', quietActions: 0,`.

`packages/engine/src/session.ts`: импорт `import type { GameResult, PlayerId, StallRule } from './types';`; сигнатура `setupNextGame(se: SessionState, playerIds: PlayerId[], deckSize: DeckSize, seed: number, stallRule: StallRule = 'forcedVidbiy'): GameSetup` и поле `stallRule,` в возвращаемом объекте.

`packages/engine/src/result.ts`: заменить `function finish(` на `export function finish(`.

- [ ] **Step 7: Правило в фазе 2**

В `packages/engine/src/phase2.ts`:
- импорт: `import { checkGameOver, finish, markOut } from './result';`
- после импортов:
```ts
...
function resolveStall(s: GameState, lastActorId: PlayerId, events: GameEvent[]): void {
  ...
  const active = seatOrderFrom(s, s.turn).filter((id) => !findPlayer(s, id)!.out);
...
Run: `npx vitest run packages/engine/test/phase2.test.ts && npm run typecheck`
Expected: PASS

- [ ] **Step 9: Фаззинг с обоими вариантами и с автоходом за всех**

В `packages/engine/test/fuzz.test.ts`:
- вернуть в `candidates` порог `0.9` (вместо `0.97`);
- добавить в импорт из `'../src'`: `autoAction`, `type StallRule`;
- в цикле сидов заменить создание партии на:
```ts
...
- [ ] **Step 10: Полный прогон**

Run: `npm test && npm run typecheck`
Expected: все тесты PASS, включая все 120 случайных сессий и 40 партий на автоходе. Записать в отчёт длительность `fuzz.test.ts`. Если какой-то сид всё ещё не завершается при `'forcedVidbiy'` — не менять правила: найти минимальную позицию и сообщить NEEDS_CONTEXT.

- [ ] **Step 11: Commit**

```bash
git add packages/engine
git commit -m "feat(engine): stalled-battle rule, public API, invariant fuzzing
